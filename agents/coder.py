...
class CoderAgent(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        return """You are a Senior Python Developer (Scientific Computing).
                ...
                13. **AUTOGRAD INTEGRITY**: When implementing gradient-based optimization (loss.backward()), the forward model computation MUST maintain the computation graph. NEVER wrap the forward pass in `torch.no_grad()` during training. NEVER call `.detach()` on the optimization variable before passing it through the forward model. Only use `torch.no_grad()` for evaluation/inference AFTER training, or for computing metrics. If you need to reduce memory, use `torch.utils.checkpoint` sparingly, not `no_grad`.
                14. **SEGMENT SCOPE**: When asked to implement a FUNCTION (target_type='function'), output ONLY that function's code (def + body). Do NOT include import statements, class definitions, or if __name__ blocks. These will be merged into the existing file by the system. Including extra class headers causes duplicate/nested class definitions."""
    ...
    def _build_user_prompt(self, context: Dict[str, Any]) -> str:
        ...
        if context.get('knowledge_context'):
            ...
            skill_instruction = "\n### 🧠 SKILL UTILIZATION (MANDATORY — HIGHEST PRIORITY)\nReview the 'RELEVANT SKILLS' above carefully. These come from VALIDATED implementations that PASSED evaluation.\n"
            skill_instruction += "⚠️ **Skills take PRIORITY over the Plan when they provide specific implementation details.**\n\n"
            skill_instruction += "- When skills provide SPECIFIC formulas, coordinate conventions, operator implementations, or numerical constants, you MUST follow them EXACTLY. Do NOT substitute your own formulas.\n"
            skill_instruction += "- When skills specify an operator ordering, normalization convention, sign convention, or hyperparameters, implement EXACTLY as described — even if the Plan says something different.\n"
            skill_instruction += "- When skills contain code snippets, COPY the logic directly. These are proven to work.\n"
            skill_instruction += "- **Conflict resolution**: If the Plan contradicts a skill on a SPECIFIC implementation detail (formula, sign, ordering, coordinates), FOLLOW THE SKILL. The Plan is generated by an LLM that may have errors; skills are extracted from validated code.\n"
            skill_instruction += "- **DO NOT** add features not mentioned in skills or the Plan (no gradient checkpointing, no lr auto-calibration, no complex scheduling).\n"
...
- NUMERICAL PRECISION: For iterative physics forward models, use the precision specified in the Plan (typically float64/complex128). Lower precision may cause NaN in long iterative chains.
- LEARNING RATE: If the Plan specifies a fixed learning rate value, use EXACTLY that value. Do NOT implement learning rate auto-calibration, adaptive scheduling, or gradient-based lr estimation. Use a simple fixed lr with `x -= lr * grad`.
- FORMULA FIDELITY: Before implementing each operator/update rule, copy the EXACT formula from the Plan.
  Verify: (a) sign correct (+ vs -)? (b) normalization factor included? (c) smooth vs hard mask as Plan specifies?
- SMOOTH vs HARD MASKS: If Plan says "smooth damping/sigmoid/exponential", use continuous functions (exp, sigmoid, tanh).
  NEVER substitute binary masks (torch.where with hard cutoff) unless Plan explicitly says "binary".
- CRITICAL IMPLEMENTATION CHECKLIST: If the Plan includes a [Critical Implementation Checklist],
  implement each item EXACTLY as specified. These are the most error-prone details.
...
"""
    ...
    def _check_plan_compliance(self, code: str, plan: str) -> list:
        """Check generated code against Plan for common implementation errors (generic)."""
        warnings = []
        plan_lower = plan.lower()

        # 1. Sign check: if plan mentions subtraction/minus for update, verify code has -=
        if re.search(r'(minus|subtract|negative sign|\-=)', plan_lower):
            if '+=' in code and '-=' not in code:
                warnings.append("Plan specifies subtraction/minus but code only uses +=. Check operator signs.")

        # 2. Normalization check: if plan mentions normalize/per-pixel/n_pixels
        if re.search(r'(normaliz|per.pixel|n_pixels|divide by.*pixel)', plan_lower):
            if 'n_pixels' not in code and '/ n' not in code and '/n_' not in code and '/ num' not in code:
                warnings.append("Plan specifies normalization but code may be missing division by pixel count.")

        # 3. Smooth mask check: if plan mentions sigmoid/smooth/exponential damping
        if re.search(r'(sigmoid|smooth.*mask|exponential.*damp|soft.*cutoff)', plan_lower):
            if 'torch.where' in code and ('zeros_like' in code or 'ones_like' in code):
                warnings.append("Plan specifies smooth masks but code uses torch.where with hard cutoffs. Use sigmoid/exp instead.")
    ...
    def implement_and_merge(self, context: Dict[str, Any]) -> str:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # Plan compliance check (generic: sign, normalization, smooth masks)
                plan_text = context.get('plan', '')
                if plan_text and attempt < max_retries - 1:
                    compliance_warnings = self._check_plan_compliance(result, plan_text)
                    if compliance_warnings:
                        print(f"[CoderAgent] Plan compliance warnings: {compliance_warnings}")
                        current_feedback = context_for_llm.get('feedback')
                        if current_feedback is None:
                            current_feedback = ""
                        context_for_llm['feedback'] = (
                            current_feedback +
                            "\n\nPLAN COMPLIANCE WARNINGS:\n" +
                            "\n".join(f"- {w}" for w in compliance_warnings) +
                            "\nFix these issues. Re-read the Plan carefully and follow its exact formulas and conventions."
                        )
                        full_code = result  # use the improved version as base for next attempt
                        continue
